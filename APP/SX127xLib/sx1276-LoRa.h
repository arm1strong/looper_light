 ...
 * RegFifoTxBaseAddr
 */
#define RFLR_FIFOTXBASEADDR                         0x00 // Default
...
//高度封装SX1278 目的用于多个LoRa的场景
typedef struct 
{
    char spi_nmb;                    //使用哪个SPI

    void (*setReset)(char nmb, char * level);  //复位LoRa
    char (*readDIO0)(char nmb);          //读取LoRa数据
    void (*setSPINSS)(char nmb, char level);
    void (*writeSPIData)(char nmb, char * data, uint16_t size);
    void (*readSPIData)(char nmb, char * data, uint16_t size);
